...
void Display_List_Buku(int page, int data_per_page, int data, Perpus buku[]){
    int stock = 0;
    for(int i = page * data_per_page; i < (page + 1) * data_per_page; i++){
        if(i == 0){
            printf("+----------------------------------------------------------------------------------------------------------------------------------------+\n");
...
void List_Buku(){
    ...
    do{
        ...
        printf("Pilihan anda (1-3) : %d", pilihan);
        scanf("%d", &pilihan);
        ...
        case 1 :
            if(page < total_page - 1){
                ...
                Clear_System();
                Display_List_Buku(page, data_per_page, data, buku);
                printf("\nAnda sudah berada di page terakhir.\n\n");
            }
            break;
        ...
        case 2 :
            if(page > 0){
                ...
                Clear_System();
                Display_List_Buku(page, data_per_page, data, buku);
                printf("\nAnda sudah berada di page pertama.\n\n");
            }
            break;
        ...
        default:
            Clear_System();
            Display_List_Buku(page, data_per_page, data, buku);
            printf("Pilihan anda tidak valid. Silahkan ulangi (1-3).\n\n");
            break;
        }
